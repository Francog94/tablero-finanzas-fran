          ...
          <>
            ...
            >
              <div style={cardStyle}>
                ...
                <div style={{ ...valueStyle, color: "#f87171", fontSize: "26px", lineHeight: 1.2, wordBreak: "break-word" }}>{money(totalGastos, monedaActiva)}</div>
              </div>

              <div style={cardStyle}>
                <h3 style={{ ...labelStyle, textTransform: "uppercase", letterSpacing: ".04em" }}>Ingresos del período</h3>
                <div style={{ ...valueStyle, color: "#34d399", fontSize: "26px", lineHeight: 1.2, wordBreak: "break-word" }}>{money(totalIngresos, monedaActiva)}</div>
              </div>
            </div>
            ...
            <div style={{ ...cardStyle, marginBottom: "16px", textAlign: "center" }}>
              ...
              <div style={{ display: "flex", justifyContent: "center", width: "100%", overflow: "hidden" }}>
                {(() => {
                  const total = porCategoria.reduce((acc, item) => acc + Number(item.value || 0), 0);
                  const colores = ["#3b82f6", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6"];
                  let acumulado = 0;

                  if (!porCategoria.length || total === 0) {
                    return <div style={{ color: "#94a3b8" }}>No hay datos</div>;
                  }

                  return (
                    <svg width="220" height="220" viewBox="0 0 42 42" style={{ maxWidth: "100%", height: "auto", display: "block" }}>
                      {porCategoria.slice(0, 5).map((item, i) => {
                        const porcentaje = (item.value / total) * 100;
                        const dash = `${porcentaje} ${100 - porcentaje}`;
                        const offset = 25 - acumulado;
                        acumulado += porcentaje;
                      ...
                      <circle cx="21" cy="21" r="8.5" fill="#0b1220" />
                    </svg>
                  );
                })()}
              </div>
                ...
                {porCategoria.slice(0, 5).map((item, idx) => {
                  ...
                  return (
                    <div key={item.name} style={{ display: "grid", gridTemplateColumns: "28px 1fr auto", gap: 10, alignItems: "center" }}>
                      ...
                      <strong style={{ fontSize: 13, color: "#cbd5e1", whiteSpace: "nowrap" }}>{porcentaje}% · {money(item.value, monedaActiva)}</strong>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ ...cardStyle, marginBottom: "116px", padding: "14px 16px", opacity: 0.9 }}>
